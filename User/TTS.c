...
// Random number seed
static uint8_t seed0;
static uint8_t seed1;
static uint8_t seed2;
...
static int copyToken(char token, char *dest, int x, const VOCAB * vocab)
{
	...
	const uint8_t *txt = (const uint8_t *)pgm_read_ptr(&vocab[ph].txt);
	if (pgm_read_byte(txt) == token && pgm_read_byte(txt+1) == 0) {
	    const uint8_t *src = (const uint8_t *)pgm_read_ptr(&vocab[ph].phoneme);
...
static uint8_t whitespace(char c)
{
    return (c == 0 || c == ' ' || c == ',' || c == '.' || c == '?'
	    || c == '\'' || c == '!' || c == ':' || c == '/');
}
...
static int textToPhonemes(const char *src, const VOCAB * vocab, char *dest)
{
	...
	for (unsigned int ph = 0; ph < NUM_VOCAB; ph++) {
	    ...
	    const uint8_t *text = (const uint8_t *)pgm_read_ptr(&vocab[ph].txt);
	    const uint8_t *phon = (const uint8_t *)pgm_read_ptr(&vocab[ph].phoneme);
...
static int phonemesToData(const char *textp, const PHONEME * phoneme)
{
	...
	for (unsigned int ph = 0; ph < NUM_PHONEME; ph++) {
	    ...
	    const uint8_t *ph_text = (const uint8_t *)pgm_read_ptr(&phoneme[ph].txt);
	    ...
	    const uint8_t *ph_ph = (const uint8_t *)pgm_read_ptr(&phoneme[ph].phoneme);
...
static void pause(uint8_t d)
{
//    delayMicroseconds(d*6);
    Delay_Us(d*6);
}

static void delay2(uint8_t d)
{
...
static uint8_t playTone(int pin, uint8_t soundNum, uint8_t soundPos, char pitch1, char pitch2, uint8_t count, uint8_t volume)
{
    const uint8_t *soundData = &SoundData[soundNum * 0x40];
    while (count-- > 0) {
	uint8_t s = pgm_read_byte(&soundData[soundPos & 0x3fu]);
	sound(pin, s & volume);
	pause(pitch1);
...
static void play(int pin, uint8_t duration, uint8_t soundNumber)
{
    while (duration--)
 ...
 */
void sayPhonemes(const char *textp)
{
    uint8_t phonemeIn,		// offset into text
 ...
 */
void setPitch(uint8_t pitch) { defaultPitch = pitch; }

/**
 * gets the pitch
 */
u_int8_t getPitch(void) { return defaultPitch; }
